-- Migration: 20260918000100_sync_write_v2_custom_split.sql
-- Kapsam: Görev 7.3-D4 — Custom Split DTO, Outbox Sözleşmesi ve RPC
-- 1. transactions tablosuna split_mode ve participant_shares kolonlarının eklenmesi ve backfill edilmesi.
-- 2. transactions_split_mode_check kısıtlaması (EQUAL, CUSTOM).
-- 3. sync_write_v2_before_assets RPC'sinin Custom Split, gerçek JSON sayı ve BigInt sınır doğrulamaları ile güncellenmesi.
-- 4. Eski istemci UPDATE durumunda etkin payer, katılımcılar, aktif üyelik ve tutarın birlikte doğrulanması.
-- 5. sync_write_v2_before_assets için önceki migration erişim kısıtlarının korunması (authenticated grant kaldırıldı).
...
alter table public.transactions
    ...
    add column if not exists participant_shares jsonb not null default '[]'::jsonb;

-- Geriye uyumluluk için null olan kayıtları EQUAL ve boş pay olarak güncelle
update public.transactions
   set split_mode = 'EQUAL'
 where split_mode is null;

update public.transactions
   set participant_shares = '[]'::jsonb
 where participant_shares is null;

do $$
begin
    if not exists (
        select 1 from pg_catalog.pg_constraint
        where conname = 'transactions_split_mode_check'
    ) then
        alter table public.transactions
            add constraint transactions_split_mode_check check (split_mode in ('EQUAL', 'CUSTOM'));
    end if;
end $$;


-- ============================================================================
-- 2. SYNC_WRITE_V2_BEFORE_ASSETS RPC GÜNCELLEMESİ (CUSTOM SPLIT DESTEĞİ)
-- ============================================================================

create or replace function public.sync_write_v2_before_assets(
    p_operation_id text,
    p_entity_type text,
    p_operation text,
...
declare
    ...
    v_tx_split_mode text;
    v_tx_participant_shares jsonb;
    v_tx_existing_split_mode text;
    v_tx_existing_participant_shares jsonb;
    v_tx_existing_amount_minor bigint;
    v_tx_existing_paid_by uuid;
    v_tx_existing_participants jsonb;
    v_shares_to_validate jsonb;
    v_raw_mode text;
    v_tx_amount_minor bigint;
    v_share_elem jsonb;
    v_share_user_id uuid;
    v_share_amount_str text;
    v_share_amount bigint;
    v_share_total bigint := 0;
    v_share_user_ids jsonb := '[]'::jsonb;
...
begin
    ...
    if p_entity_type = 'PROFILE' then
        ...
        if p_operation in ('CREATE', 'UPDATE') then
            ...
            -- Sayısal doğrulama: tam sayı, pozitif, gerçek JSON sayı türü, bigint sınırları
            if (p_payload -> 'amount_minor') is null or pg_catalog.jsonb_typeof(p_payload -> 'amount_minor') <> 'number' then
                raise exception using message = 'transactions.amount_minor sayı olmalıdır.';
            end if;

            if (p_payload ->> 'amount_minor') !~ '^[0-9]+$' then
                raise exception using message = 'transactions.amount_minor pozitif tam sayı olmalıdır.';
            end if;

            begin
                v_tx_amount_minor := (p_payload ->> 'amount_minor')::bigint;
            exception when others then
                raise exception using message = 'transactions.amount_minor bigint sınırını aşıyor.';
            end;

            if v_tx_amount_minor <= 0 then
                raise exception using message = 'transactions.amount_minor sıfırdan büyük olmalıdır.';
            end if;
            ...
            if v_tx_workspace_id is null then
                ...
                if v_tx_type = 'income' then
                    ...
                    -- Ortak gider işlemi: split_mode ve participant_shares belirleme ve doğrulama
                    if not (p_payload ? 'split_mode') then
                        -- split_mode alanı payload'da tamamen eksik
                        if p_operation = 'CREATE' then
                            if p_payload ? 'participant_shares' then
                                if pg_catalog.jsonb_typeof(p_payload -> 'participant_shares') <> 'array'
                                   or pg_catalog.jsonb_array_length(p_payload -> 'participant_shares') > 0 then
                                    raise exception using message = 'INVALID_SPLIT_PAYLOAD: split_mode belirtilmeden participant_shares verilemez.';
                                end if;
                            end if;
                            v_tx_split_mode := 'EQUAL';
                            v_tx_participant_shares := '[]'::jsonb;
                        else
                            -- Eski istemci UPDATE: mevcut split durumunu koru
                            select t.split_mode, t.participant_shares, t.amount_minor, t.paid_by_user_id, t.participant_user_ids
                              into v_tx_existing_split_mode, v_tx_existing_participant_shares, v_tx_existing_amount_minor,
                                   v_tx_existing_paid_by, v_tx_existing_participants
                              from public.transactions as t
                             where t.id = entity_id
                               and t.user_id = actor_id;

                            if v_tx_existing_split_mode = 'CUSTOM' then
                                v_tx_split_mode := 'CUSTOM';
                                v_shares_to_validate := v_tx_existing_participant_shares;

                                -- Etkin payer
                                if p_payload ? 'paid_by_user_id' then
                                    v_tx_paid_by_user_id := nullif(p_payload ->> 'paid_by_user_id', '')::uuid;
                                else
                                    v_tx_paid_by_user_id := v_tx_existing_paid_by;
                                end if;
                                if v_tx_paid_by_user_id is null then
                                    raise exception using message = 'Geçersiz paid_by_user_id.';
                                end if;

                                -- Etkin katılımcılar
                                if p_payload ? 'participant_user_ids' then
                                    v_tx_participant_user_ids := p_payload -> 'participant_user_ids';
                                else
                                    v_tx_participant_user_ids := v_tx_existing_participants;
                                end if;
                            else
                                v_tx_split_mode := 'EQUAL';
                                v_tx_participant_shares := '[]'::jsonb;
                        ...
                        -- split_mode alanı payload'da mevcut
                        if pg_catalog.jsonb_typeof(p_payload -> 'split_mode') = 'null' then
                            raise exception using message = 'INVALID_SPLIT_MODE: split_mode null olamaz.';
                        end if;
                        if pg_catalog.jsonb_typeof(p_payload -> 'split_mode') <> 'string' then
                            raise exception using message = 'INVALID_SPLIT_MODE: split_mode metin olmalıdır.';
                        end if;

                        v_raw_mode := trim(p_payload ->> 'split_mode');
                        if v_raw_mode = '' then
                            raise exception using message = 'INVALID_SPLIT_MODE: split_mode boş olamaz.';
                        end if;
                        if v_raw_mode not in ('EQUAL', 'CUSTOM') then
                            raise exception using message = 'INVALID_SPLIT_MODE: Desteklenmeyen split_mode: ' || v_raw_mode;
                        end if;

                        v_tx_split_mode := v_raw_mode;

                        if v_tx_split_mode = 'EQUAL' then
                            if p_payload ? 'participant_shares' then
                                if pg_catalog.jsonb_typeof(p_payload -> 'participant_shares') <> 'array' then
                                    raise exception using message = 'INVALID_SPLIT_PAYLOAD: EQUAL modunda participant_shares dizi olmalıdır.';
                                end if;
                                if pg_catalog.jsonb_array_length(p_payload -> 'participant_shares') > 0 then
                                    raise exception using message = 'INVALID_SPLIT_PAYLOAD: EQUAL modunda participant_shares boş olmalıdır.';
                                end if;
                            end if;
                            v_tx_participant_shares := '[]'::jsonb;
                        else
                            -- CUSTOM
                            if not (p_payload ? 'participant_shares') then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: CUSTOM modunda participant_shares zorunludur.';
                            end if;
                            if pg_catalog.jsonb_typeof(p_payload -> 'participant_shares') <> 'array' then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: participant_shares dizi olmalıdır.';
                            end if;
                            if pg_catalog.jsonb_array_length(p_payload -> 'participant_shares') = 0 then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: CUSTOM modunda participant_shares boş olamaz.';
                            end if;

                            v_shares_to_validate := p_payload -> 'participant_shares';
                            v_tx_paid_by_user_id := coalesce(nullif(p_payload ->> 'paid_by_user_id', '')::uuid, actor_id);

                            if not (p_payload ? 'participant_user_ids')
                               or pg_catalog.jsonb_typeof(p_payload -> 'participant_user_ids') <> 'array'
                               or pg_catalog.jsonb_array_length(p_payload -> 'participant_user_ids') = 0 then
                                raise exception using message = 'CUSTOM_SPLIT_PARTICIPANT_SET_MISMATCH: CUSTOM modunda participant_user_ids zorunludur.';
                            end if;
                            v_tx_participant_user_ids := p_payload -> 'participant_user_ids';
                        end if;
                    end if;

                    -- EQUAL modu için katılımcı ve ödeyen doğrulaması
                    if v_tx_split_mode = 'EQUAL' then
                    ...
                    -- CUSTOM modu için katılımcılar, paylar, aktif üyelik ve toplam doğrulaması
                    elsif v_tx_split_mode = 'CUSTOM' then
                        v_share_total := 0;
                        v_share_count := 0;
                        v_share_user_ids := '[]'::jsonb;

                        for v_share_elem in select * from pg_catalog.jsonb_array_elements(v_shares_to_validate) loop
                            if pg_catalog.jsonb_typeof(v_share_elem) <> 'object' then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Pay elemanı nesne olmalıdır.';
                            end if;

                            v_share_user_id := nullif(coalesce(v_share_elem ->> 'user_id', v_share_elem ->> 'userId'), '')::uuid;
                            if v_share_user_id is null then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payı için geçerli bir kullanıcı kimliği zorunludur.';
                            end if;

                            if not (v_share_elem ? 'amount_minor') and not (v_share_elem ? 'amountMinor') then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payı için amount_minor zorunludur.';
                            end if;

                            if (v_share_elem ? 'amount_minor' and pg_catalog.jsonb_typeof(v_share_elem -> 'amount_minor') <> 'number')
                               or (v_share_elem ? 'amountMinor' and pg_catalog.jsonb_typeof(v_share_elem -> 'amountMinor') <> 'number') then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payı amount_minor sayı olmalıdır.';
                            end if;

                            v_share_amount_str := coalesce(v_share_elem ->> 'amount_minor', v_share_elem ->> 'amountMinor');
                            if v_share_amount_str is null or not (v_share_amount_str ~ '^[0-9]+$') then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payı tam sayı olmalıdır.';
                            end if;

                            begin
                                v_share_amount := v_share_amount_str::bigint;
                            exception when others then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payı bigint sınırını aşıyor.';
                            end;

                            if v_share_amount < 0 then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payı negatif olamaz.';
                            end if;

                            if v_share_user_id <> v_tx_paid_by_user_id and v_share_amount <= 0 then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Ödeyen dışındaki katılımcıların payı sıfırdan büyük olmalıdır: ' || v_share_user_id::text;
                            end if;

                            if v_share_user_ids ? v_share_user_id::text then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: participant_shares içinde tekrarlanan kullanıcı: ' || v_share_user_id::text;
                            end if;

                            -- Taşma kontrolü (Long.MAX_VALUE = 9223372036854775807)
                            if (9223372036854775807::bigint - v_share_total) < v_share_amount then
                                raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payları toplamında Long taşması (overflow).';
                            end if;

                            v_share_user_ids := v_share_user_ids || pg_catalog.jsonb_build_array(v_share_user_id::text);
                            v_share_total := v_share_total + v_share_amount;
                            v_share_count := v_share_count + 1;
                        end loop;

                        if not (v_share_user_ids ? v_tx_paid_by_user_id::text) then
                            raise exception using message = 'INVALID_SPLIT_PAYLOAD: Ödeme yapan kişi pay listesinde olmalıdır: ' || v_tx_paid_by_user_id::text;
                        end if;

                        if v_share_total <> v_tx_amount_minor then
                            raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcı payları toplamı (' || v_share_total || ') işlem tutarına (' || v_tx_amount_minor || ') eşit olmalıdır.';
                        end if;

                        -- Katılımcı kümesi birebir kontrolü (CUSTOM_SPLIT_PARTICIPANT_SET_MISMATCH)
                        select count(*), count(distinct elem)
                          into v_tx_participant_count, v_tx_distinct_participant_count
                          from pg_catalog.jsonb_array_elements_text(v_tx_participant_user_ids) as elem;

                        if v_tx_participant_count <> v_share_count or v_tx_distinct_participant_count <> v_share_count then
                            raise exception using message = 'CUSTOM_SPLIT_PARTICIPANT_SET_MISMATCH: Katılımcı sayısı ile pay sayısı uyuşmuyor.';
                        end if;

                        if exists (
                            select 1
                              from pg_catalog.jsonb_array_elements_text(v_tx_participant_user_ids) as elem
                             where not (v_share_user_ids ? elem)
                        ) then
                            raise exception using message = 'CUSTOM_SPLIT_PARTICIPANT_SET_MISMATCH: Katılımcı kümesi ile pay sahipleri kümesi uyuşmuyor.';
                        end if;

                        -- Aktif üyelik kontrolü (tüm katılımcılar)
                        select count(*)
                          into v_tx_valid_member_count
                          from public.workspace_members as m
                         where m.workspace_id = v_tx_workspace_id
                           ...
                           and m.user_id in (
                               select elem::uuid
                                 from pg_catalog.jsonb_array_elements_text(v_share_user_ids) as elem
                           );

                        if v_tx_valid_member_count <> v_share_count then
                            raise exception using message = 'INVALID_SPLIT_PAYLOAD: Katılımcılar arasında çalışma alanının aktif üyesi olmayan kullanıcılar var.';
                        end if;

                        -- Ödeyen kişi aktif üye kontrolü
                        if not exists (
                            select 1 from public.workspace_members
                             where workspace_id = v_tx_workspace_id and user_id = v_tx_paid_by_user_id and deleted_at is null
                        ) then
                            raise exception using message = 'INVALID_SPLIT_PAYLOAD: Ödeme yapan kişi çalışma alanının aktif üyesi olmalıdır.';
                        end if;

                        -- Kanonik sıralama (user_id artan)
                        select coalesce(pg_catalog.jsonb_agg(
                            pg_catalog.jsonb_build_object(
                                'user_id', coalesce(elem ->> 'user_id', elem ->> 'userId'),
                                'amount_minor', (coalesce(elem ->> 'amount_minor', elem ->> 'amountMinor'))::bigint
                            ) order by coalesce(elem ->> 'user_id', elem ->> 'userId') asc
                        ), '[]'::jsonb)
                        into v_tx_participant_shares
                        from pg_catalog.jsonb_array_elements(v_shares_to_validate) as elem;
                    end if;
                end if;
            end if;
        end if;
        ...
        if p_operation = 'CREATE' then
            insert into public.transactions (
                id, user_id, workspace_id, category_id, amount_minor, currency,
                type, description, payment_method, transaction_date, receipt_path,
                installment_number, total_installments, installment_group_id,
                paid_by_user_id, participant_user_ids, split_mode, participant_shares, created_at
            ) values (
                entity_id,
                actor_id,
                v_tx_workspace_id,
                ...
                v_tx_split_mode,
                v_tx_participant_shares,
                coalesce((p_payload ->> 'created_at')::timestamptz, pg_catalog.timezone('utc'::text, pg_catalog.now()))
            )
            on conflict (id) do nothing
            returning pg_catalog.to_jsonb(transactions.*) into written_row;
        elsif p_operation = 'DELETE' then
            if p_payload ? 'user_id' and nullif(p_payload ->> 'user_id', '')::uuid <> actor_id then
                raise insufficient_privilege using message = 'İşlem sahipliği reddedildi.';
            end if;
            ...
            update public.transactions
               set workspace_id = v_tx_workspace_id,
                   category_id = v_tx_category_id,
                   amount_minor = v_tx_amount_minor,
                   currency = p_payload ->> 'currency',
                   type = v_tx_type,
                   ...
                   paid_by_user_id = v_tx_paid_by_user_id,
                   participant_user_ids = v_tx_participant_user_ids,
                   split_mode = v_tx_split_mode,
                   participant_shares = v_tx_participant_shares,
                   deleted_at = null
             where id = entity_id
               and user_id = actor_id
               and version = p_base_version
        ...
        if p_operation = 'CREATE' then
            v_sub_next_renewal_date := coalesce(v_sub_next_renewal_date, v_sub_start_date);
