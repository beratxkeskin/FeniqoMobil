-- TASLAK: Sync Write V2 Goals & Debts Şeması ve Aggregate RPC Genişletmesi. Yalnız yerel/staging için.
begin;

-- 1. Kişisel V2 Goals Tablosu (public.goals)
...
create table public.goals (
    ...
    constraint goals_workspace_personal_check
        check (workspace_id is null),

    constraint goals_target_amount_minor_positive
        check (target_amount_minor > 0),

    constraint goals_current_amount_minor_non_negative
        check (current_amount_minor >= 0),

    constraint goals_currency_check
        check (currency in ('TRY', 'USD', 'EUR')),

    constraint goals_color_hex_format_check
        check (color_hex ~* '^#[0-9a-f]{6}$'),

    constraint goals_name_check
        check (length(trim(name)) > 0 and length(name) <= 500),

    constraint goals_version_positive
        check (version > 0)
);

create index idx_goals_sync_cursor
    on public.goals (updated_at, id);

create index idx_goals_active_owner
    on public.goals (user_id, updated_at, id)
    where deleted_at is null;

create trigger goals_set_server_metadata
...
create policy goals_select_personal_v1
    on public.goals for select to authenticated
    using (user_id = (select auth.uid()) and workspace_id is null);


-- 2. Kişisel V2 Goal Contributions Tablosu (public.goal_contributions)
create table public.goal_contributions (
    ...
    constraint goal_contributions_amount_minor_positive
        check (amount_minor > 0),

    constraint goal_contributions_currency_check
        check (currency in ('TRY', 'USD', 'EUR')),

    constraint goal_contributions_direction_check
        check (direction in ('ADD', 'REMOVE')),

    constraint goal_contributions_note_check
        check (note is null or (length(trim(note)) > 0 and length(note) <= 500)),

    constraint goal_contributions_version_positive
        check (version > 0)
);

create index idx_goal_contributions_sync_cursor
    on public.goal_contributions (updated_at, id);

create index idx_goal_contributions_goal
    on public.goal_contributions (goal_id, updated_at, id);

create index idx_goal_contributions_active
    on public.goal_contributions (goal_id, updated_at, id)
    where deleted_at is null;

create trigger goal_contributions_set_server_metadata
...
revoke all on table public.goal_contributions from authenticated;

grant select on table public.goal_contributions to authenticated;

create policy goal_contributions_select_personal_v1
...
-- 3. Kişisel V2 Debts Tablosu (public.debts)
create table public.debts (
    ...
    constraint debts_workspace_personal_check
        check (workspace_id is null),

    constraint debts_title_check
        check (length(trim(title)) > 0 and length(title) <= 500),

    constraint debts_amount_minor_positive
        check (amount_minor > 0),

    constraint debts_currency_check
        check (currency in ('TRY', 'USD', 'EUR')),

    constraint debts_type_check
        check (type in ('DEBT', 'RECEIVABLE')),

    constraint debts_status_check
        check (status in ('OPEN', 'SETTLED')),

    constraint debts_description_check
        check (description is null or (length(trim(description)) > 0 and length(description) <= 500)),

    constraint debts_version_positive
        check (version > 0)
);

create index idx_debts_sync_cursor
    on public.debts (updated_at, id);

create index idx_debts_active_owner
    on public.debts (user_id, updated_at, id)
    where deleted_at is null;

create trigger debts_set_server_metadata
...
revoke all on table public.debts from authenticated;

grant select on table public.debts to authenticated;

create policy debts_select_personal_v1
    on public.debts for select to authenticated
    using (user_id = (select auth.uid()) and workspace_id is null);


-- 4. Kişisel V2 Debt Payments Tablosu (public.debt_payments)
create table public.debt_payments (
    ...
    constraint debt_payments_amount_minor_positive
        check (amount_minor > 0),

    constraint debt_payments_currency_check
        check (currency in ('TRY', 'USD', 'EUR')),

    constraint debt_payments_version_positive
        check (version > 0)
);

create index idx_debt_payments_sync_cursor
    on public.debt_payments (updated_at, id);

create index idx_debt_payments_debt
    on public.debt_payments (debt_id, updated_at, id);

create index idx_debt_payments_active
    on public.debt_payments (debt_id, updated_at, id)
    where deleted_at is null;

create trigger debt_payments_set_server_metadata
...
revoke all on table public.debt_payments from authenticated;

grant select on table public.debt_payments to authenticated;

create policy debt_payments_select_personal_v1
...
-- 5. sync_operations_receipts entity_type ve base_version kısıtlamalarını güncelle
...
alter table public.sync_operations_receipts
    drop constraint if exists sync_operations_receipts_base_version_check;

alter table public.sync_operations_receipts
    add constraint sync_operations_receipts_base_version_check
    check (
        (operation_type = 'CREATE' and entity_type not in ('GOAL_CONTRIBUTION', 'DEBT_PAYMENT') and base_version is null) or
        (operation_type = 'CREATE' and entity_type in ('GOAL_CONTRIBUTION', 'DEBT_PAYMENT') and base_version >= 1) or
        (operation_type in ('UPDATE', 'DELETE') and base_version >= 1)
    );


-- 6. sync_write_v2 RPC fonksiyonunu GOAL, GOAL_CONTRIBUTION, DEBT, DEBT_PAYMENT destekleyecek şekilde yeniden tanımla
