-- TASLAK: Sync Write V2 Abonelik Şeması ve SUBSCRIPTION RPC Genişletmesi. Yalnız yerel/staging için.
begin;

-- 1. Kişisel V2 Abonelik Tablosu (public.subscriptions)
...
create table public.subscriptions (
    ...
    constraint subscriptions_workspace_personal_check
        check (workspace_id is null),

    constraint subscriptions_amount_minor_positive
    ...
    constraint subscriptions_interval_positive
        check (interval > 0),

    constraint subscriptions_date_order_check
        check (end_date is null or end_date >= start_date),

    constraint subscriptions_next_renewal_date_order_check
        check (next_renewal_date >= start_date and (end_date is null or next_renewal_date <= end_date)),

    constraint subscriptions_name_check
        check (length(trim(name)) > 0 and length(name) <= 500),

    constraint subscriptions_version_positive
        check (version > 0)
);

-- Artımlı senkronizasyon cursor ve sorgu indeksleri
create index idx_subscriptions_sync_cursor
    on public.subscriptions (updated_at, id);

create index idx_subscriptions_active_owner
...
create trigger subscriptions_set_server_metadata
...
create policy subscriptions_select_personal_v1
    on public.subscriptions for select to authenticated
    using (user_id = (select auth.uid()) and workspace_id is null);


-- 2. sync_operations_receipts entity_type kısıtlamasını SUBSCRIPTION kabul edecek şekilde güncelle
...
alter table public.sync_operations_receipts
    ...
    check (entity_type in ('PROFILE', 'CATEGORY', 'TRANSACTION', 'BUDGET', 'RECURRING_TRANSACTION', 'SUBSCRIPTION'));


-- 3. sync_write_v2 RPC fonksiyonunu SUBSCRIPTION destekleyecek şekilde yeniden tanımla
...
begin
    ...
    if p_entity_type not in ('PROFILE', 'CATEGORY', 'TRANSACTION', 'BUDGET', 'RECURRING_TRANSACTION', 'SUBSCRIPTION') then
