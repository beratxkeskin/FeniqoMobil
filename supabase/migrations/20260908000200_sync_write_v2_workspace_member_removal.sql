-- Migration: Workspace Member Removal & RLS Inbound Tombstone (E12-F / E12-F0)
-- Kapsam:
-- 1. workspace_members SELECT politikasının dar kapsamda güncellenmesi:
--    - Çıkarılmış üyenin yalnız kendi tombstone satırını pull edebilmesi (user_id = auth.uid())
--    - Aktif üyelerin yalnızca diğer aktif üyeleri görebilmesi
--    - Çıkarılmış üyenin diğer üyeleri veya finans verilerini görememesi
-- 2. sync_write_v2 WORKSPACE_MEMBER DELETE davranışının genişletilmesi:
--    - baseVersion zorunlu ve pozitiflik kontrolü
--    - Aktif OWNER'ın aktif EDITOR/VIEWER üyeyi çıkarma yetkisi
--    - OWNER'ın çıkarılmasının fail-closed engellenmesi
--    - Atomik tombstone ve idempotency receipt kaydı

begin;

-- ============================================================================
-- 1. WORKSPACE_MEMBERS RLS SELECT POLİTİKASI GÜNCELLEMESİ (E12-F0)
-- ============================================================================

drop policy if exists workspace_members_select_member_v1 on public.workspace_members;
drop policy if exists workspace_members_select_v2 on public.workspace_members;

create policy workspace_members_select_v2
...
-- ============================================================================
-- 2. SYNC_WRITE_V2 RPC GÜNCELLEMESİ (WORKSPACE_MEMBER DELETE)
-- ============================================================================
