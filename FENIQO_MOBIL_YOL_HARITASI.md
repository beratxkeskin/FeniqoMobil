...
- [x] **LocalMutationDao Güvenli Toparlanması:** Yalnız `DEFINITIVE_REJECTION` durumunda `HARD_DELETE` veya yeni UUID ile temiz `CREATE` kuyruklanır. `AMBIGUOUS_RESULT` durumunda eski `operation_id` ve payload korunur, yeni `CREATE` üretilmez; ardıl mutasyonlar `predecessor_operation_id` ve `is_blocked = true` ile bağlanarak idempotent replay ve uzak zombi kayıt kalmaması güvenceye alınır.
...
| No | Senaryo | Durum | Kanıt ve Kalan İş |
|---|---|---|---|
| **S1** | Çevrimdışı kategori ve gelir/gider oluşturma | **KISMİ / AÇIK** | Cihaz B'de çevrimdışı Market işlemi (450,00 TL) oluşturma yerel olarak kanıtlandı. Çevrimdışı özel kategori oluşturma kanıtı ve uzak ACK henüz kanıtlanmadığı için KISMİ/AÇIK. |
| **S2** | Çevrimdışıyken force-stop ve yeniden açma, Room/outbox kalıcılığı | **KISMİ (B yerel geçti / A AÇIK)** | Cihaz B'de uçak modunda force-stop sonrası yeniden açılışta Room/outbox kalıcılığı korundu. Yalnızca Cihaz B'de yapıldığı ve Cihaz A üzerinde doğrulanmadığı için özgün cihaz kabulü tamamlanmış sayılmaz, iki cihaz kabulü AÇIK. |
| **S3** | Ağ geri gelince sync, ACK ve idempotency | **KALDI** | Staging Supabase'deki eksik kanonik kategoriler nedeniyle outbox işlemi `FAILED` oldu. Güvenli fail-closed migration hazırlandı ancak staging'e henüz UYGULANMADI (onay bekliyor); gerçek staging ACK alınana kadar KALDI. |
| **S4** | İkinci cihaz initial pull ve çift yönlü güncelleme | **AÇIK** | Cihaz A aktif kullanımda olduğu için gerçek iki cihazlı initial pull ve çift yönlü güncelleme kanıtı henüz üretilmedi; AÇIK. |
| **S5** | Soft delete/tombstone ve zombie olmaması | **AÇIK** | Yerel testte ve Room'da soft-delete/successor zinciri doğrulandı ancak gerçek iki cihaz ve staging remote üzerinde tombstone/zombie olmaması doğrulanana kadar AÇIK. |
| **S6** | İki ayrı kayıtla KEEP_LOCAL ve KEEP_REMOTE conflict çözümü | **AÇIK** | Yerel çakışma çözücü mantığı mevcut; gerçek iki cihaz eş zamanlı mutasyonu ve iki ayrı kayıtla conflict çözümü kanıtı yapılana kadar AÇIK. |
| **S7** | Görünen adın Header/Profil/restart/ikinci cihaz eşitlemesi | **KISMİ / AÇIK** | B cihazında ad değişikliği Room ve outbox'a atomik yazıldı. Ancak restart sonrası alınan görsel splash ekranı olduğundan profil/görünen ad kanıtı olarak geçersizdir; taze auth oturumu/parola kanıtı ve Cihaz A eşitlemesi henüz yapılmadığından KISMİ/AÇIK. |
| **S8** | Dört giriş noktası, tekil ve 3+ taksit, başarı navigasyonu, gerçek transaction ID ve THIS_AND_FOLLOWING | **KISMİ (B yerel alt adımları geçti / uzak AÇIK)** | Başarı ekranı kapatıldığında form yığını temizlenmesi ve doğru transaction ID ile açılış ile kişisel taksit oluşturma ve `THIS_AND_FOLLOWING` ile takip eden taksitlerin Room'dan atomik silinmesi B yerelinde GEÇTİ. Uzak staging eşitlemesi ve Cihaz A yansıması AÇIK. |

### Kişisel Bilgiler görünen ad kaydı düzeltmesi (2026-09-20)
...
### İşlem sonrası navigasyon düzeltmesi (2026-09-19)
...
> 2026-09-16 — Görev 7.3-D2 (Custom Split Saf Domain, Doğrulama ve Settlement): Platformdan bağımsız domain modeli (`TransactionSplitMode`, `TransactionParticipantShare`), `TransactionCommand` ve `Transaction` geriye uyumlu entegrasyonu, tipli `TransactionValidationRules` doğrulama ve normalizasyon kuralları, `EqualSplitCalculator` saf kuruş remainder motoru, `WorkspaceSettlementCalculator` ve `ObserveWorkspaceSettlementUseCase` fail-closed dışlama (`hasExcludedExpenses = true`) kuralları tamamlandı.
> 2026-09-18 — Görev 7.3-D4 (Custom Split DTO, Outbox Sözleşmesi ve RPC) ve Görev 7.3-D5 (Custom Split UI Entegrasyonu): Outbox/RPC sync zinciri, katı çakışma çözücü, pglast doğrulama ve 71 SQL sözleşme senaryosu tamamlandı. D4 test koşucusu sabit paroladan arındırıldı. UI katmanında `TransactionValidationRules.validateSplit`, `MoneyAmountParser`, `Long` kuruş aritmetiği ve payer 0-pay kuralı ile entegrasyon sağlandı; CUSTOM modu yalnızca ortak workspace giderlerinde sunuldu (`isSharedExpense`), kişisel ve gelir işlemlerinde EQUAL'a normalize edildi; Room'dan gelen mevcut paylar eksiksiz yüklendi ve yalnız açıklama düzenlemelerinde paylar korundu; `FormSnapshot` ve `hasUnsavedChanges` sıra bağımsız Map karşılaştırmasıyla donatıldı; Ayrıntılar dialogunda İptal ana formu değiştirmeksizin taslağı bırakırken Uygula split alanlarını ViewModel'e aktardı; katılımcı ekleme/çıkarma ve payer değişimlerinde tutarlar otomatik yeniden dağıtılmadı, eski payer'ın sıfır payı yeni durumda görünür hata (`SPLIT_CUSTOM_NON_PAYER_ZERO_SHARE_NOT_ALLOWED`) yapıldı ve aktif üyelik kontrolleri korundu; "Kalan tutarı ödeyene aktar" eylemi Long taşma korumalı ve negatif/aşım blokajlı olarak uygulandı; teknik kodlar gizlenerek kullanıcı dostu etiketler, semantik TalkBack açıklamaları ve kişi bazlı hata metinleri sağlandı. Senaryo 1-5 emülatör manuel akışlarıyla, Senaryo 6 izole Room Flow → ViewModel → Compose enstrümante test paketiyle (`CustomSplitMembershipDepartureAndroidTest`), Senaryo 7 (7a tek büyük giriş, 7b iki payın toplam kuruş sınırı, 7c 12 kişilik liste ve açık klavyede buton erişilebilirliği; `CustomSplitBoundaryAndLayoutAndroidTest`) testleriyle doğrulanıp ekran kanıtları `docs/MANUEL_KABUL_7_3_D5.md` dosyasına kaydedildi; manuel kabul tamamlandı.
...
**İlerleme notu (2026-09-20 — Profil Merkezi Rota ve Durum Entegrasyonu):** Profil Merkezi (`ProfileScreen`) üzerindeki eski "Yakında" etiketleri kaldırıldı; Profil Özet kartı dahil tüm satırlar bağımsız type-safe Compose rotalarına (`AccountRoute`, `PersonalInfoRoute`, `WorkspacePickerRoute`, `AppearanceRoute`, `NotificationsSettingsRoute`, `LanguageRegionRoute`, `SecurityPrivacyRoute`, `DataManagementRoute`, `LegalInfoRoute`, `HelpAboutRoute`) bağlandı. `UserSettingsRepository` üzerinden reaktif dil ve para birimi tercihi (`AppLanguage · Currency`) etikete bağlandı; `SyncDisplaySeverity`'nin 7 durumu (`UP_TO_DATE`, `PENDING`, `OFFLINE`, `CHECKING`, `SYNCING`, `ERROR`, `CONFLICT`) semantik yüzey ve ikonlarla eksiksiz eşlendi; null profil durumu açık yükleme/boş durum ve oturum e-postası fallback'i ile ayrıştırılarak sonsuz yükleme riski giderildi; yardım makalesi ve lisansların dürüst "Hazırlanıyor" durumu korundu. Robolectric Compose testleri, ViewModel testleri, `:sharedLogic:testAndroidHostTest`, `:sharedLogic:compileKotlinIosSimulatorArm64` ve `:androidApp:assembleDebug` başarıyla doğrulandı.

**İlerleme notu (2026-09-19 — Profil ve Ayarlar A1–B4 Alt Akışları ve Manuel Kabul):** Onaylı tasarım panolarına (A1–A4 Profil & Hesap Akışları, B1–B4 Yardım, Hukuki Bilgiler & Geri Bildirim) sadık kalınarak:
- Pano A1: Hesabım Ekranı (`AccountScreen`) diskten güvenli yerel avatar okuma, dairesel maske, kamera rozetiyle Pano A1 bottom sheet tasarımı (kamera, galeri, mevcut fotoğrafı kaldır) ve "Fotoğraflarınız yalnızca bu cihazda yerel olarak saklanır" dürüstlüğüyle uygulandı. E-posta satırında 3 durumlu doğrulama rozeti bağlandı.
- Pano A2: Fotoğraf Önizleme ve Kırpma (`PhotoPreviewScreen`) KMP ortak UI katmanında `ImageBitmap` ile 1:1 dairesel maske, 3x3 kılavuz çizgileri, pinch-to-zoom/pan ve zoom slider ile geliştirildi. Android katmanında `UserAvatarManager` regex ve canonicalPath korumasıyla path traversal'a karşı güvenli hâle getirildi; yeni görsel önce geçici `.tmp` dosyasına yazılıp `Files.move(REPLACE_EXISTING)` ile atomik biçimde mevcut avatarla değiştirildi. `FileProvider` yalnız dar kapsamlı `cache/camera/` dizinini paylaşmaktadır.
- Pano A3: E-posta Durumu ve Yönetimi (`EmailSettingsScreen`) `EmailVerificationStatus` (VERIFIED, PENDING_VERIFICATION, UNKNOWN) 3 durumlu mimariyle oluşturuldu; Supabase'den bilgi alınamadığında false varsayılmayıp UNKNOWN bırakıldı; oturum tazeleme (`refreshEmailVerificationStatus`), doğrulama e-postası tekrar gönderme (`resendEmailVerification`) ve yerel profil boş olduğunda auth oturum e-postasına düşen güvenli fallback (`displayEmail`) uygulandı.
- Pano A4: Hesap Silme Hazırlığı (`AuthSettingsScreens`) kırmızı şeffaf uyarı kutusu ve 3 koruyucu adımla (Verilerini dışa aktar [makbuz ve ortak alanların dahil olmadığını dürüstçe açıklayan], Ortak alanlarını kontrol et, Eşitlenmemiş değişiklikleri incele) uygulandı; Destek ile İletişime Geç butonu Pano B4'e bağlandı. Sahte veya kontrolsüz doğrudan silme butonu konulmadı.
- Pano B1: Yardım Merkezi (`HelpCenterScreen`) arama çubuğu, 4 kategori kartı, 5 popüler makale ve destek iletişim butonuyla kodlandı.
- Pano B2: Makale Durumu (`HelpArticleStatusScreen`) dinamik başlık, "Hazırlanıyor" rozeti ve geri bildirim yönlendirmesiyle uygulandı.
- Pano B3: Gizlilik ve Hukuki Bilgiler (`LegalInfoScreen`) genişletilebilir Gizlilik Politikası, Kullanım Koşulları ve KVKK metinleri ile; açık kaynak lisansları gerçek metinler derlenene kadar dürüst "Hazırlanıyor" rozetiyle korundu.
- Pano B4: Geri Bildirim (`HelpAboutScreen`) sahte "Bildiriminiz alındı" Toast'u kesinlikle kaldırılarak, iptalde taslağı koruyan, 0/1000 karakter sayaçlı, ekran görüntüsü ekleme ve `FLAG_GRANT_READ_URI_PERMISSION` paylaşım intent'li olarak uygulandı. "Hata bildir" girişi doğrudan Hata butonunu seçili ve kırmızı vurgulu (`#D32F2F`) getirecek şekilde ayarlandı.
Tüm hedefli birim testleri (25/25), `:sharedLogic:testAndroidHostTest`, `:androidApp:testDebugUnitTest`, `:sharedLogic:compileKotlinIosSimulatorArm64` ve `:androidApp:assembleDebug` ile fiziksel Android cihaz (`SM-A715F - 13`) manuel kabul adımları (A1 avatar galeri/kamera/kırpma/kayıt, A3 e-posta ve doğrulama, A4 silme hazırlığı, B1-B3 yardım ve hukuki bilgiler, B4 hata bildirimi) başarıyla tamamlandı.

**İlerleme notu (2026-09-17 — Raporlar ve Analizler 01–26 Tüm Onaylı Tasarım Panoları):** 6 onaylı tasarım panosunun tamamındaki 26 ekran (01–04 Raporlar Ana ve Dönem Özeti, 05–08 Kategori Dağılımı ve Detayı, 09–12 Nakit Akışı ve Dönem Karşılaştırması, 13–16 Harcama Takvimi, Bütçe, Abonelik ve Borç/Alacak Özetleri, 17–20 Gelecek Dönem Tahmini ve Finansal İçgörüler, 21–26 Filtreler, Özel Tarih Aralığı, Çoklu Para Birimi ve Sistem Durumları) eksiksiz uygulandı. Clean Architecture, Room SSOT ve `Long` para kuralları korundu; saf KMP `DetailedReportUseCases` use-case ailesi ve `ReportsViewModel` bağlandı. Özel Compose Canvas grafikleri (`TrendDualLineChart`, `WeeklyDualBarChart`, `ReportDonutChart`, `BidirectionalCashFlowChart`), MoreHub "Yakında" rozetinin kaldırılıp gerçek `ReportsRoute`'a bağlanması ve `maskAmounts` tam tutar maskelemesi tamamlandı. Hedefli birim/host testleri (`:androidApp:testDebugUnitTest`, `:sharedLogic:testAndroidHostTest`, `:sharedUI:testAndroidHostTest`), iOS simülatör derlemesi (`:sharedLogic:compileKotlinIosSimulatorArm64`) ve Android debug APK derlemesi (`:androidApp:assembleDebug`) başarıyla doğrulandı.
...
- [x] Custom Split (Tutara göre özel ortak dağıtım) DTO, Outbox Sözleşmesi ve RPC (Görev 7.3-D4): DTO split_mode ve participant_shares alanları, fail-closed snake_case DTO ↔ domain mappers (CUSTOM modunda payer ile katılımcı tamamlamama, eksik/null/boş/bilinmeyen split_mode ayrımı), outbox ve katı EquivalentConflictResolver entegrasyonu (yalnız geçerli sözleşmeler, bozuk/tutarsız payları reddetme), tek atomik 20260918000100_sync_write_v2_custom_split.sql migration (sync_write_v2_before_assets erişim kısıtlarının korunması, commit sonrası wrapper bloğu kaldırıldı, gerçek JSON number/tam sayı/bigint sınırları ve kontrollü overflow kontrolü, eski istemci CUSTOM UPDATE yolunda etkin payer/katılımcılar/aktif üyelik ve tutar toplamı doğrulaması, EQUAL modunda yanlış JSON türlerini reddetme, subscriptions frequency regresyon düzeltmesi, goals/debts/workspace bloklarının kanonik şemaya geri yüklenmesi), Senaryo 71 (71.1-71.12) beklenen hata mesajı kontrollü SQL sözleşme senaryoları, 27 migration zinciri ve sözleşme testleri yerel PostgreSQL 18 (ICU tr-TR, UTF-8) izole ortamında `supabase/tests/run_local_sync_contract_test.py` ile exit code 0 ile doğrulandı (RPC testleri postgres rolüyle, doğrudan RLS Senaryo 64'te authenticated rolüyle). Room kalıcılık testleri (TransactionSplitRoomPersistenceTest, entity+outbox, ACK, pull, conflict snapshot, fail-closed cursor koruması). UI entegrasyonuna (D5) geçilmedi.
- [ ] Taksitli işlem oluşturma ve silme davranışı.
- [ ] Makbuz bağlama için UI hazırlığı.

**İlerleme notu (2026-09-18 — Görev 7.3-D4 Custom Split DTO, Outbox Sözleşmesi ve RPC):** DTO, outbox, RPC, Room kalıcılık testleri ve izole PostgreSQL 18 runtime testi tamamlandı; yerel geliştirme kabulü verildi (UI entegrasyonuna — Görev 7.3-D5 — henüz geçilmedi).
1. `TransactionDto` içine `@SerialName("split_mode") val splitMode: String? = null` ve `@SerialName("participant_shares") val participantShares: List<TransactionParticipantShareDto>? = null` eklendi; explicit JSON null ve eksik alan ayrımı güvenli biçimde sağlandı.
2. `CoreFinanceRemoteMappers` içinde katı ve fail-closed doğrulama kuralları güncellendi: CUSTOM mapper yolunda boş `participantUserIds` payer ile tamamlanmaz (`RemoteMappingException` fırlatılır); eski EQUAL fallback davranışı CUSTOM'dan ayrıldı; null mod + pay yoksa `EQUAL` kabul edilir; null mod + pay varsa veya `EQUAL` + pay varsa veya `CUSTOM` + pay yoksa `RemoteMappingException` fırlatılır; negatif pay, payer harici sıfır pay, boş/geçersiz kullanıcı, tekrarlı kullanıcı, payer'ın pay listesinde olmaması, `participantUserIds` kümesi uyuşmazlığı (`CUSTOM_SPLIT_PARTICIPANT_SET_MISMATCH`), toplam eşitliği ve `Long` taşması fail-closed kontrol edilir.
3. `EquivalentConflictResolver` yalnız geçerli split sözleşmelerini eşdeğer kabul edecek şekilde sıkılaştırıldı; bozuk EQUAL/CUSTOM, boş/bilinmeyen mod ve tutarsız paylarda `false` döner.
4. `20260918000100_sync_write_v2_custom_split.sql` migration'ı tek atomik işlemde güncellendi: `sync_write_v2_before_assets` için önceki migration'ın erişim kısıtları (`revoke ... from public, anon, authenticated`) korundu, `authenticated` grant'i kaldırıldı; commit sonrası koşullu wrapper bloğu kaldırıldı; gerçek JSON sayı türü (`jsonb_typeof = 'number'`), tam sayı formatı (`^[0-9]+$`), bigint sınırları ve kontrollü toplama taşması kontrolü eklendi; eski istemci CUSTOM UPDATE yolunda etkin payer, katılımcılar, aktif üyelik ve tutar toplamı re-validate edildi; EQUAL pay alanındaki geçersiz JSON türleri reddedildi; TRANSACTION DELETE yükü için CREATE/UPDATE alan zorunlulukları kaldırıldı; `subscriptions` tablosundaki `billing_cycle` regresyonu giderilip şemadaki kanonik `frequency` sütununa dönüldü ve yenileme tarihi korumaları sağlandı; `goals`, `debts`, `workspace` blokları 20260908000200 kanonik DDL sözleşmesine geri yüklendi.
5. `supabase/tests/sync_write_v2_contract.sql` ve `supabase/tests/run_local_sync_contract_test.py`: Senaryo 71 (71.1–71.12) test senaryoları güncellendi: SQL negatif testlerinde genel exception yerine beklenen spesifik hata mesajı (`SQLERRM`) denetlendi; 27 migration zinciri ve sözleşme testleri yerel PostgreSQL 18 (ICU `tr-TR`, UTF-8) üzerinde sıfır hata ve exit code 0 ile tamamlandı; RPC testleri `postgres` süper kullanıcı rolüyle, doğrudan RLS izolasyonu ise Senaryo 64'te `authenticated` rolüne düşürülerek doğrulandı.
6. Gerçek Room kalıcılığı: `TransactionSplitRoomPersistenceTest` androidHostTest testiyle entity + outbox, ACK (`ackV2Execution`), pull (`IncrementalRemoteSync.pullFor`), conflict snapshot ve bozuk uzak kayıtta cursor'un ilerletilmemesi Room üzerinde %100 doğrulandı.
...
| Tarih | Adım | Not |
|---|---|---|
| 2026-09-20 | Profil Merkezi Rota ve Durum Entegrasyonu | Profil Merkezi (`ProfileScreen`) üzerindeki eski "Yakında" etiketleri kaldırıldı; Profil Özet kartı dahil tüm satırlar bağımsız type-safe Compose rotalarına (`AccountRoute`, `PersonalInfoRoute`, `WorkspacePickerRoute`, `AppearanceRoute`, `NotificationsSettingsRoute`, `LanguageRegionRoute`, `SecurityPrivacyRoute`, `DataManagementRoute`, `LegalInfoRoute`, `HelpAboutRoute`) bağlandı. `UserSettingsRepository` üzerinden reaktif dil ve para birimi tercihi (`AppLanguage · Currency`) bağlandı; `SyncDisplaySeverity`'nin 7 durumu (`UP_TO_DATE`, `PENDING`, `OFFLINE`, `CHECKING`, `SYNCING`, `ERROR`, `CONFLICT`) semantik yüzey/ikonlarla eşlendi; null profil durumu açık yükleme/boş durum ve oturum e-postası fallback'i ile ayrıştırılarak sonsuz yükleme engellendi; yardım makalesi ve lisansların dürüst "Hazırlanıyor" durumu korundu. Robolectric Compose testleri, ViewModel testleri, `:sharedLogic:testAndroidHostTest`, `:sharedLogic:compileKotlinIosSimulatorArm64` ve `:androidApp:assembleDebug` ile doğrulandı. |
| 2026-09-19 | Profil ve Ayarlar (A1–B4 Tam Kabul) | Onaylı tasarım panolarına (A1–A4 Profil & Hesap Akışları, B1–B4 Yardım, Hukuki Bilgiler & Geri Bildirim) sadık kalınarak tam akışlar ve cihaz kabulü tamamlandı: A1-A2 Güvenli yerel avatar (UserAvatarManager regex/canonicalPath korumalı, atomic replace, safeMove yedekleme garantisi, CameraX çalışma zamanı izinleri ve isolated draft cache ile galeri/kamera kırpma); A3 E-posta yönetimi (3 durumlu EmailVerificationStatus, session email fallback'li displayEmail, doğrulama yenileme ve tekrar gönderme); A4 Hesap silme hazırlığı (veri dışa aktarma kapsam uyarısı, ortak alan kontrolü, sync inceleme ve destek yönlendirmesi); B1-B3 Yardım merkezi (arama filtreleme, makale hazırlanıyor durumu, akordeon hukuki metinler ve hazırlanıyor lisans rozeti); B4 Geri bildirim formu (taslak korumalı, 0/1000 sayaçlı, ekran görüntüsü ekleme, paylaşım menüsü intent'i, Hata bildirimi için doğrudan kırmızı vurgulu [#D32F2F] Hata butonu seçimi). Birim testleri (25/25), KMP host testleri, iOS Simulator derlemesi ve Android fiziksel cihaz (SM-A715F) adım adım manuel testi başarıyla tamamlandı. |
| 2026-09-18 | 7.3-D5 (Custom Split UI Entegrasyonu) | Manuel kabul tamamlandı (`docs/MANUEL_KABUL_7_3_D5.md`): CUSTOM + taksit kombinasyonunda sessiz eşit kayıt engellendi, UI ve submit katmanında açıklayıcı Türkçe hatayla (`SPLIT_CUSTOM_NOT_SUPPORTED_WITH_INSTALLMENT`) use case çağrısı üretmeden durduruldu, taksiti kapatma veya eşit dağıtımı seçme imkânı verildi ve paylar silinmedi; dağıtılan/kalan/fazla tutar gösterimleri `Long` taşması ile `Money.MAX_AMOUNT_MINOR` sınırına karşı güvenli yapıldı, sınır dışı tutarlar `Money` constructor'ına verilmeden açıklayıcı durum metinleriyle sunuldu; "Tam Eşleşti" yalnız tüm seçili paylar eksiksiz ve geçerliyken gösterilecek şekilde sıkılaştırıldı, taslak içindeki kişi bazlı hatalar güncel girdilerden anlık hesaplandı (`validateDraftParticipantShares`) ve `customShareErrors` oluştuğunda ayrıntılar akordeonunun otomatik açılması sağlandı; CUSTOM formunda üyelik Flow'unun payer/katılımcıları sessizce değiştirmesi engellendi, yeni form varsayılanı ile kullanıcının veya Room'un dağılımı ayrıştırıldı, üye ayrıldığında seçim ve tutar korunup inaktif (`(Ayrıldı)`) işaretlenerek düzeltme yapılmadan submit edilmesi engellendi. Senaryo 1-5 emülatör manuel akışlarıyla, Senaryo 6 izole Room Flow → ViewModel → Compose enstrümante test paketiyle (`CustomSplitMembershipDepartureAndroidTest` / XML kaydedildi), Senaryo 7 (7a tek büyük giriş, 7b iki payın toplam kuruş sınırı, 7c 12 kişilik liste ve açık klavyede buton erişilebilirliği; `CustomSplitBoundaryAndLayoutAndroidTest`) testleriyle doğrulanıp ekran kanıtları `docs/MANUEL_KABUL_7_3_D5.md` dosyasına kaydedildi. |
| 2026-09-18 | Rapor kategori adı hata düzeltmesi | Rapor kategori dağılımındaki tüm satırların sabit “Kategori” görünmesine neden olan sunum eşlemesi kaldırıldı. Kategori kimlikleri Room tarihsel kategori akışıyla eşlenerek uygulamadaki gerçek adlar gösteriliyor; bulunamayan tarihsel referans “Silinmiş kategori” olarak ayrıştırılıyor. Regresyon testi ve Android debug APK derlemesi başarılıdır. |
| 2026-09-18 | Geliştirici Demo Hesabı | Ayrı `com.feniqo.mobile.demo` Android paketi, yerel Deniz Demo oturumu ve tarih duyarlı altı aylık örnek veri seti tamamlandı. İşlemler, aylık bütçeler, hedef/katkılar, borç/alacak/ödemeler, abonelik ödeme ve fiyat geçmişi, tekrarlayan işlemler, varlıklar ile üç üyeli ortak alan normal repository ve Room entity+outbox akışından hazırlanır. Demo INTERNET izni içermez; Supabase `.invalid` yapılandırmasına sabittir, Realtime/arka plan sync kapalıdır. Uygulama içi sıfırlama yalnız demo sandbox'ını temizler. Demo senaryo ve gerçek Room entegrasyon testleri, normal debug ve demo APK derlemeleri başarılıdır; production/staging'e dokunulmadı. |
...
| 2026-09-17 | Raporlar • Filtreler ve Sistem Durumları (Paneller 21–26 Altyapısı) | Uygulandı, doğrulamalar geçti: Onaylı 6. Pano (21–26) tasarımına sadık kalınarak Rapor Filtreleri (sheet & draft state), Özel Tarih Aralığı (interaktif takvim & hızlı seçimler), Çoklu Para Birimi (TRY/USD/EUR/GBP, bağımsız kartlar, birleştirilmemiş toplamlar), Boş Durumlar (Henüz rapor yok, Filtrelerde sonuç yok) ve Sistem Durumları (Loading, offlineRoom korumalı senkronizasyon uyarısı, hata ve retry) tamamlandı. Currency enum'ına geriye dönük uyumlu GBP eklendi (Room String uyumluluğu korundu). Clean Architecture & MVVM uygulandı: domain katmanında ReportFilter, ReportPeriodPreset, ReportTypeFilter, ReportDateRange ve use-case'ler (ObserveFilteredReport, ObserveMultiCurrencyReport, ObserveReportAvailability, ObserveReportSyncStatus); ReportsViewModel SyncOverview domain modelini tüketir; ReportsUiState (ContentState, ConnectionState, FilterUiState) çevrimdışıyken yerel Success raporlarını gizlemez. 01–20 ana görsel rapor ekranları henüz kodlanmadığı için MoreHub'daki 'Yakında' etiketi korunmuştur ve özellik kısmi altyapı olarak belgelenmiştir. Hedefli sharedLogic, sharedUI ve androidApp testleri, debug APK ve iOS simulator derlemesi başarıyla geçti; Supabase canlı/staging ortamına müdahale edilmedi. |
| 2026-09-21 | Staging Migration ve V1 Çekirdek Kabul Doğrulaması (M1–M4) | `20260921000100_seed_extended_canonical_categories.sql` migration'ı proje sahibi açık onayıyla yalnız `rxfaiynkhaxrksosxvxp` staging projesine uygulandı; production'a dokunulmadı. M1 Sözleşme, M2 Yerel Postgres 16 Senaryo, M3 Staging Uygulaması ve M4 Staging Sonrası Doğrulama GEÇTİ (27 aktif varsayılan sistem kategorisi [9 gelir + 18 gider], 1120/1121 tombstone version=2, 12 yeni kanonik kategori eksiksiz ve katalog dışı fazlalık 0, kullanıcı kategorileri korundu). İki cihaz (Cihaz A: SM-A715F, Cihaz B: sdk_gphone16k_x86_64) şu anda ADB üzerinde bağlı olmadığından özgün S1–S8 kabul senaryoları sözleşme kuralı gereği sahte başarı üretilmeden dürüstçe AÇIK bırakıldı. Tam regresyon (sharedLogic 1499 test, androidApp 686 test, sharedUI 313 test, assembleDebug, compileKotlinIosSimulatorArm64 ve 16 senaryolu python SQL testi) %100 geçti. Git commit/push yapılmadı. |
