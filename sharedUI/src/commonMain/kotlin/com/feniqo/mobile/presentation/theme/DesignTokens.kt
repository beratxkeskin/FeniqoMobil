package com.feniqo.mobile.presentation.theme

import androidx.compose.ui.text.TextStyle
import androidx.compose.ui.text.font.FontFamily
import androidx.compose.ui.text.font.FontWeight
import androidx.compose.ui.unit.dp
import androidx.compose.ui.unit.sp

/** Ortak ekran ve bileşen boşlukları için 4dp tabanlı ölçek. */
object FeniqoSpacing {
    val ExtraSmall = 4.dp
    val Small = 8.dp
    val Medium = 12.dp
    val Large = 16.dp
    val LargePlus = 20.dp
    val ExtraLarge = 24.dp
    val Screen = 32.dp
}

/** Feniqo arayüzündeki yuvarlak köşe değerleri. */
object FeniqoRadius {
    val Small = 12.dp
    val Medium = 16.dp
    val Large = 24.dp
}

/** Finansal durumları ifade eden, iş kuralından bağımsız görsel renkler. */
object FeniqoStatusColor {
    val Success = FeniqoEmerald
    val Warning = FeniqoWarning
    val Error = FeniqoExpense
    val Info = FeniqoInfo
}

/** Erişilebilir etkileşim alanları için ortak alt sınırlar. */
object FeniqoTouchTarget {
    val Minimum = 48.dp
    val PrimaryAction = 56.dp
}

/** Tekrar kullanılabilir warm-luxury ve display tipografi belirteçleri. */
object FeniqoTypographyTokens {
    val DisplayTitle = TextStyle(
        fontFamily = FontFamily.Serif,
        fontWeight = FontWeight.Bold,
        fontSize = 28.sp,
        lineHeight = 34.sp,
        letterSpacing = (-0.5).sp,
    )
}
